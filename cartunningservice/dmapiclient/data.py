...
class DataAPIClient(BaseAPIClient):
    ...
    def list_task_by_user(self, user_uuid, page, per_page):
        params = {}
        if page is not None:
            params['page'] = page
        if per_page is not None:
        ...
        try:
            response = self._get(
                '/api/1.0/users/{}/tasks/'.format(user_uuid),
            params=params)
    ...
    def create_task(self, user_uuid, type_id, brand_id, model_id, engine_id, ecu_id):
        try:
            response = self._post(
                '/api/1.0/users/{}/tasks/'.format(user_uuid),
                data={
